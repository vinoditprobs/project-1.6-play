const S={screen:1,choice:null,coins:0,time:30,running:false,timer:null,form:null,branch:{},started:false,introVisible:false,enteringLift:false,arrivingLobby:false,assetsReady:false };
...
function sceneFor(n){const scene=$('scene');const inside=n>4&&n<27;scene.replaceChildren(cloneTemplate(document,inside?'#scene-inside':'#scene-lobby'));if(S.enteringLift)scene.querySelector('.lift-inside')?.classList.add('lift-entry');const lobby=scene.querySelector('.lobby-image');if(lobby){if(S.assetsReady)lobby.classList.add('close-door');if(S.arrivingLobby){lobby.classList.add('lobby-arrival');lobby.addEventListener('animationend',()=>{S.arrivingLobby=false;lobby.classList.remove('lobby-arrival')},{once:true})}}}
