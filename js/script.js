...
function updateStats(){
  ...
  if(S.running){const timer=document.createElement('b');timer.append(timerText());leftStat.append('⌛ ',timer)}
  if(S.coins>=0){rightStat.append('Total coins: ',money())}
}
function sceneFor(n){const scene=$('scene');scene.replaceChildren(cloneTemplate(document,'#scene-lobby'));if(n>4&&n!==27)scene.replaceChildren(cloneTemplate(document,'#scene-inside'));if(n===2)scene.append(cloneTemplate(document,'#character-1'));else if(n===3)scene.append(cloneTemplate(document,'#character-2'));else if(n===6||n>=8&&n<=27){scene.append(cloneTemplate(document,'#character-1'));scene.append(cloneTemplate(document,'#character-2'));}}
